class DiceThrow {
    constructor(factor, multiplier, sides, modifier) {
        if (typeof factor === 'string') {
            const match = factor.match(/(?:(\d+)\s*\*\s*)?(\d*)d(\d+)(?:\s*([\+\-]\s*\d+))?/i);
            this.factor = parseInt(match[1]);
            this.multiplier = parseInt(match[2]);
            this.sides = parseInt(match[3]);
            this.modifier = typeof(match[4]) === 'string' ? parseInt( match[4].replace(/\s/g, '') ) : 0;
        } else {
    ...
    toString() {
        ...
        out += `d${this.sides}`;
        if (this.modifier !== 0) out += `${this.modifier > 0 ? ` + ${this.modifier}` : ` - ${this.modifier}`}`;
